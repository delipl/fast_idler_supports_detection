...
int load_test_cloud(pcl::PointCloud<PointT>::Ptr cloud, const char* file_name = "test_cloud.pcd") {
    ...
    return pcl::io::loadPCDFile<T>(path, *cloud);
}
template <typename T>
void save_cloud(const std::string& name, pcl::PointCloud<PointT>::Ptr cloud) {
...
TEST(ObjectDetectionTest, ToCharts) {
    ...
    auto cloud = pcl_utils::remove_far_points_from_ros2bag_converter_bug<Point>(cloud_raw, 5.0);
    save_cloud("short.pcd", cloud);
    // Ground Filtering
    Eigen::Vector3d normal_vec;
    double ground_height;
    auto filtered_ground_clouds = object_detection.filter_ground_and_get_normal_and_height(
        cloud, pcl::SACMODEL_PLANE, 800, 0.07, std::ref(normal_vec), std::ref(ground_height));
