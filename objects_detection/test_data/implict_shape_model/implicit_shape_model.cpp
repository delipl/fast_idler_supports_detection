...
int main(int argc, char** argv) {
    ...
    normal_estimator.setRadiusSearch(4.0);
    ...
    for (unsigned int i_cloud = 0; i_cloud < number_of_training_clouds - 1; i_cloud++) {
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(argv[i_cloud * 2 + 1], *tr_cloud) == -1) return (-1);
    ...
    unsigned int testing_class = static_cast<unsigned int>(strtol(argv[6], 0, 10));
    pcl::PointCloud<pcl::PointXYZ>::Ptr testing_cloud(new pcl::PointCloud<pcl::PointXYZ>());
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(argv[5], *testing_cloud) == -1) return (-1);
    ...
    std::cout << "Testing normals size: " << testing_normals->size() << std::endl;
    std::cout << "Testing class id: " << testing_class << std::endl;
