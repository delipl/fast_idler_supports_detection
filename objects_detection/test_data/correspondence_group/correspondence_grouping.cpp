...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Model normals size: " << model_normals->size() << std::endl;
    ...
    for (std::size_t i = 0; i < scene_descriptors->size(); ++i) {
        ...
        if (found_neighs == 1 &&
            neigh_sqr_dists[0] < 0.01f)  //  add match only if the squared descriptor distance is less than 0.25 (SHOT
