...
CloudPtr OutlierRemoval::statistical_pcl(CloudPtr cloud, uint k, double stddev_mul_thresh) const {
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("statistical_outlier_removal"), "Got: "
        << cloud->points.size() << " points.");
    RCLCPP_INFO_STREAM(rclcpp::get_logger("statistical_outlier_removal"), "Removed: "
        << removed_points_count << " points.");
    return filtered;
}