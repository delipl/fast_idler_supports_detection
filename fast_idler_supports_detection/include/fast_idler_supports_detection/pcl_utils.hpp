...
template <typename PointT>
typename pcl::PointCloud<PointT>::Ptr convert_point_cloud2_to_cloud_ptr(rclcppCloudSharedPtr pc2) {
...
typename pcl::PointCloud<PointT>::Ptr merge_clouds(const std::vector<typename pcl::PointCloud<PointT>::Ptr> &clouds) {
    typename pcl::PointCloud<PointT>::Ptr merged_clouds(new typename pcl::PointCloud<PointT>);
